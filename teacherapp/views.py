...
def add_details(request):
    if request.method=='POST':
        ...
        product=ProductDetails(firstname=fname,lastname=lname,age=age,dob=dob,gender=gend,uid=uid,admission=ad,department=dept,nof=nof,nom=nom,email=email,phone=ph,address=add,image=image)
        print("Save data...")
        product.save()
...
def edit_Product_Details(request,pk):
    if request.method == 'POST':
        ...
        pr.uid = request.POST.get('uid')
        pr.admission = request.POST.get('admission')
